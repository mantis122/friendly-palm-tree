...
class ChapterOneDirector(private val flags: QuestFlags) {
        ...
        if (theme == ImaginationTheme.FANTASY) {
            return when {
                ...
                flags.has(DEFEATED_MOON_BLOB) && !flags.has(SIGIL_QUEST_ACCEPTED) -> {
                    ...
                    "Nice swing! But while we were fighting, the Moon Goblins stole our silver sigil. I think we hid a little brass key in one of our treasure chests."
                }

                flags.has(SIGIL_QUEST_ACCEPTED) && !flags.has(GATE_UNLOCKED) ->
                    if (flags.has("OPENED_FIRST_CHEST")) {
                        "That's the key! The Moon Gate is beside the Goblin Fort—let's see if it fits."
                    } else {
                        "The Moon Gate is locked. We should check that treasure chest for the little brass key we hid."
                    }
    ...
    fun companionGateReaction(): String? {
        ...
        return if (flags.has("OPENED_FIRST_CHEST")) {
            "Sir Mia taps the little lock with her shield. “That's it! Try the brass key we found in the chest.”"
        } else {
            "Sir Mia taps the little lock with her shield. “Locked. Didn't we hide a little brass key in one of the treasure chests?”"
        }
    }
